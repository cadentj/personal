...
const colors = ["#DE4C3F", "#DE4C3F", "#DE4C3F", "#DE4C3F", "#DE4C3F", "#DE4C3F"]
...
function Items({ w = 0.7, gap = 0.15, setTitles, setColors }) {
    ...
    return (
        <ScrollControls horizontal damping={10} pages={(width - xW + urls.length * xW) / width}>
            <Minimap />
            <Scroll>
                {urls.map((url, i) => <Item key={i} index={i} position={[xW * i, 0, 0]} scale={[w, 4, 1]} url={url} setTitles={setTitles} setColors={setColors} />) /* prettier-ignore */}
            </Scroll>
        </ScrollControls>
    )
}
...
function Screen(props) {
    ...
    return (
        ...
        <>
            ...
            <Items setTitles={setTitles} setColors={props.setColors} />
        </>
    )
}
